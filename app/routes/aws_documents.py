...
async def upload_document_with_aws_analysis(
    ...
    try:
        ...
        try:
            ...
            analysis_metadata = {"ocr": {}, "comprehend": {}}
        ...
        raise HTTPException(
            status_code=500, detail=f"Error procesando documento: {str(e)}"
        )
...
async def create_user_folders(
    user_token: dict = Depends(require_no_temp_password_token),
):
    try:
        aws_service = AWSService()
        result = await aws_service.create_user_folders(user_token["uid"])
        return {"message": "Carpetas creadas exitosamente", "folders": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_category_folder(
    ...
    try:
        aws_service = AWSService()
        folder_path = await aws_service.create_category_folder(
            user_token["uid"], category
        )
        return {
