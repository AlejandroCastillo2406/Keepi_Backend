"""Recursos del usuario autenticado bajo `/me` (expediente del paciente)."""
...
router = APIRouter(prefix="/me", tags=["Mi expediente"])
...
async def put_my_medical_record(
    ...
    """Misma semántica que PATCH (clientes que usan PUT)."""
    svc = MedicalRecordService(db)
    try:
        return svc.patch_by_patient(patient, body)
    except ValueError as e:
