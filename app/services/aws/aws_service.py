...
class AWSService:
    ...
    async def upload_to_s3_with_folder(self, file_data: bytes, file_name: str, user_id: str, folder_name: str) -> str:
        ...
        try:
            ...
            # Convertir categoría a ASCII para metadatos de S3
    ...
    def _to_ascii_safe(self, text: str) -> str:
        ...
        Ejemplo: "Recibo Eléctrico" -> "Recibo Electrico"
        """
        try:
            # Normalizar el texto (NFD descompone caracteres acentuados)
            # Por ejemplo: é -> e + ́ (e + acento agudo)
            normalized = unicodedata.normalize('NFD', text)
            # Eliminar marcas diacríticas (acentos) y convertir a ASCII
            ascii_text = normalized.encode('ascii', 'ignore').decode('ascii')
            # Si el texto quedó vacío después de la conversión, usar versión básica
            if not ascii_text.strip():
                # Fallback: reemplazar caracteres no ASCII con espacios
