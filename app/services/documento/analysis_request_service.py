...
class AnalysisRequestService:
    ...
    def _resolve_patient_folder_name(
        self, *, patient_id: UUID, patient_name: Optional[str] = None
    ) -> str:
        if (patient_name or "").strip():
            return patient_name.strip()
        patient = self._users.get_by_id_plain(patient_id)
        return patient_folder_label(patient)

    async def _save_analysis_to_doctor_s3(
        self,
        *,
        doctor_id: UUID,
        patient_id: UUID,
        ...
        """Guarda el estudio en S3 del doctor: users/{doctor_id}/{paciente}/Analisis/."""
        doctor_uid = str(doctor_id)
        folder_label = self._resolve_patient_folder_name(
    ...
    async def upload_via_public_token(
        ...
        *,
        ...
        try:
            ...
            document = await self._save_analysis_to_doctor_s3(
    ...
    async def doctor_upload_and_complete(
        ...
        *,
        ...
        try:
            ...
            document = await self._save_analysis_to_doctor_s3(
                doctor_id=analysis_req.doctor_id,
                patient_id=analysis_req.patient_id,
                patient_name=patient_folder_label(patient),
                analysis_req=analysis_req,
                content=content,
                filename=filename,
                mime_type=mime_type,
