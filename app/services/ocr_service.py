...
class OCRService:
    ...
    async def extract_text_from_document(self, file_path: str, file_type: str) -> Dict[str, Any]:
        """
        Extrae texto y metadatos de un documento usando AWS Textract
        """
        try:
    ...
    async def _process_pdf(self, file_path: str) -> Dict[str, Any]:
        """Procesa archivos PDF con Textract"""
        try:
            # Leer el archivo PDF
            with open(file_path, 'rb') as file:
                pdf_bytes = file.read()
            
            # Usar Textract para procesar el PDF
    ...
    async def _process_image(self, file_path: str) -> Dict[str, Any]:
        """Procesa archivos de imagen con Textract"""
        try:
            # Leer la imagen
            with open(file_path, 'rb') as file:
                image_bytes = file.read()
            
            # Usar Textract para procesar la imagen
            response = self.textract_client.analyze_document(
    ...
    def _parse_textract_response(self, response: Dict) -> Dict[str, Any]:
        """Parsea la respuesta de Textract y extrae información relevante"""
        extracted_data = {
            'full_text': '',
            'tables': [],
            'forms': [],
        ...
        extracted_data['emails'] = re.findall(email_pattern, extracted_data['full_text'])
        ...
        if cells:
            ...
            for row in range(1, max_row + 1):
                ...
                for col in range(1, max_col + 1):
                    cell = next((c for c in cells if c.get('RowIndex') == row and c.get('ColumnIndex') == col), None)
                    if cell:
    ...
    def _get_cell_text(self, cell_block: Dict, all_blocks: List[Dict]) -> str:
        """Obtiene el texto de una celda de tabla"""
        cell_text = ''
    ...
    async def extract_document_metadata(self, file_path: str, file_type: str) -> Dict[str, Any]:
        """
        Extrae metadatos específicos del documento para categorización
        """
    ...
    async def extract_document_metadata_from_bytes(self, file_content: bytes, file_type: str) -> Dict[str, Any]:
        """
        Extrae metadatos específicos del documento desde bytes para categorización
        """
        ocr_data = await self.extract_text_from_bytes(file_content, file_type)
    ...
    async def extract_text_from_bytes(self, file_content: bytes, file_type: str) -> Dict[str, Any]:
        """
        Extrae texto y metadatos de un documento desde bytes usando AWS Textract
        """
        try:
            if file_type.lower() in ['pdf']:
                return await self._process_pdf_bytes(file_content)
            elif file_type.lower() in ['jpg', 'jpeg', 'png', 'tiff', 'bmp']:
                return await self._process_image_bytes(file_content)
            else:
                raise ValueError(f"Tipo de archivo no soportado: {file_type}")
    ...
    async def _process_pdf_bytes(self, pdf_bytes: bytes) -> Dict[str, Any]:
        """Procesa archivos PDF desde bytes con Textract"""
        try:
            # Usar Textract para procesar el PDF
            response = self.textract_client.analyze_document(
                Document={'Bytes': pdf_bytes},
    ...
    async def _process_image_bytes(self, image_bytes: bytes) -> Dict[str, Any]:
        """Procesa imágenes desde bytes con Textract"""
        try:
            # Usar Textract para procesar la imagen
            response = self.textract_client.analyze_document(
                Document={'Bytes': image_bytes},
    ...
    def _detect_document_type(self, ocr_data: Dict) -> str:
        """Detecta el tipo de documento basado en el contenido extraído"""
        ...
        if any(word in text for word in ['factura', 'invoice', 'bill', 'cobro']):
            return 'factura'
        elif any(word in text for word in ['contrato', 'contract', 'acuerdo', 'agreement']):
            return 'contrato'
        elif any(word in text for word in ['identificación', 'id', 'cedula', 'passport', 'licencia']):
            return 'identificación'
        elif any(word in text for word in ['recibo', 'receipt', 'comprobante', 'voucher']):
            return 'recibo'
        elif any(word in text for word in ['certificado', 'certificate', 'diploma', 'título']):
            return 'certificado'
